...
class AvoidanceController:
    ...
    def __init__(self, target_distance_cm=12.5):
        ...
        self.speed_out_sm = float(self.min_speed)
        self.prev_cmd = self.CMD_STRAIGHT
        self.acc_enabled = True # Cờ bật/tắt tính năng bám đuôi

    def process_logic(self, detections, width, height, dynamic_head_deg, use_dynamic_roi, detector):
        # ==========================================
        # 1. TÍNH TOÁN VÙNG CẤM (DYNAMIC ROI) - ĐÃ THU NHỎ
        # ==========================================
        # Thu hẹp 2 bên: Giảm từ 0.40 -> 0.30 (đáy) và 0.15 -> 0.08 (đỉnh)
        hw_bottom, hw_top = int(width * 0.30), int(width * 0.08)
        
        # Rút ngắn tầm nhìn: Tăng y_top từ 0.45 lên 0.60 
        # (Số càng lớn thì đỉnh hình thang càng bị tụt xuống gần xe mình hơn)
        ...
        if min_distance != 9999.0:
            # ==========================================
            # LỖI Ở ĐÂY: Bạn đã quên thêm "and self.acc_enabled"
            # Mình đã thêm vào để khi tắt ACC, nó sẽ nhảy xuống lệnh Phanh Gấp (else)
            # ==========================================
            if is_dynamic_target and self.acc_enabled:
                ...
                # ==========================================
                # PLAN B: CHẾ ĐỘ PHANH KHẨN CẤP AN TOÀN
                # (Chạy khi gặp red_car/green_car, HOẶC khi bạn tắt bám đuôi xe 'car')
                # ==========================================
                if min_distance <= 15.0:  # Mình nới lên 15.0cm cho an toàn khi tắt PID nhé
                    cmd = self.CMD_STOP
                self.speed_out_sm = float(self.min_speed)
        else:
            self.speed_out_sm = -1 
